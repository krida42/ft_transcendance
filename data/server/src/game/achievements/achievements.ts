...
export class Achievement {
  ...
  async checkFirstAchievement(user: User, condition: any, condition2:any, achievementName: string): Promise<void> {
    try {
      ...
      if (gamesPlayed === 1 || gamesPlayed2 === 1) {
        const firstAchievement = await Achievements.findOne({ where: { name: achievementName } });
        if (firstAchievement) {
  ...
  async checkFirstGame(user: User): Promise<void> {
    ...
    const achievementName = 'First game';
    await this.checkFirstAchievement(user, condition, condition2, achievementName);
  }
  ...
  async checkFirstLose(user: User): Promise<void> {
    ...
    const achievementName = 'First lose';
    await this.checkFirstAchievement(user, condition, condition2, achievementName);
  }
  ...
  async checkFirstWin(user: User): Promise<void> {
    ...
    const achievementName = 'First win';
    await this.checkFirstAchievement(user, condition, condition2, achievementName);
  }
  ...
  async checkFirstDraw(user: User): Promise<void> {
    ...
    const achievementName = 'First draw';
    await this.checkFirstAchievement(user, condition, condition2, achievementName);
  }

  async checkBarryAllen(user: User): Promise<void> {
    try {
      const gamesPlayed = await Games.count({ where: { [Op.or]: [{ player1_id: user.public_id }, { player2_id: user.public_id }], time: { [Op.lt]: 60} }});
      if (gamesPlayed === 1) {
        const achievement = await Achievements.findOne({ where: { name: 'Barry Allen' } });
        if (achievement) {
  ...
  async checkOlderThanInternet(user: User): Promise<void> {
    try {
      const gamesPlayed = await Games.count({ where: { [Op.or]: [{ player1_id: user.public_id }, { player2_id: user.public_id }], time: { [Op.gt]: 270 } }});
      if (gamesPlayed === 1) {
        const achievement = await Achievements.findOne({ where: { name: 'Older than Internet' } });
        if (achievement) {
          await this.unlock(user, achievement);
        }
      }
  ...
  async checkWinner(user: User): Promise<void> {
    try {
      const games = await Games.findAll({
        where: {
          [Op.or]: [{ player1_id: user.public_id }, { player2_id: user.public_id }],
        },
      ...
      for (let i = 0; i < games.length; i++) {
        ...
        const isUserLoser = (isUserPlayer1 && game.score1 > game.score2) || (!isUserPlayer1 && game.score2 > game.score1);
        if (isUserLoser) {
          consecutiveWins++;
        } else {
          break;
      ...
      if (consecutiveWins === 5) {
        const achievement = await Achievements.findOne({ where: { name: 'WINNER' } });
        if (achievement) {
          await this.unlock(user, achievement);
        }
      }
  ...
  async checkLoser(user: User): Promise<void> {
    try {
      ...
      if (consecutiveLosses === 5) {
        const achievement = await Achievements.findOne({ where: { name: 'LOSERRRRRRRRRR' } });
        if (achievement) {
          await this.unlock(user, achievement);
        }
      }
